...
namespace SylphyHorn.Services
{
	...
	{
		internal DesktopState(Guid id, string cName, string title, int position, bool isCurrent, bool individualWallpaperEnabled, string wallpaperPath, WallpaperPosition wallpaperPosition)
		{
			this.Id = id;
			this.CName = cName;
			this.Title = title;
	...
	public sealed class DesktopControlService : IDesktopControlService, IDisposable
	{
		...
		{
			internal RememberedWallpaper(string path, WallpaperPosition position) { this.Path = path; this.Position = position; }
		...
		public DesktopState GetDesktop(DesktopSelector selector) => ResolveDesktop(this.GetState(), selector, out _);
		public GlobalWallpaperState GetIndividualWallpapersState() => new GlobalWallpaperState(this.GetState().IndividualWallpapersEnabled);
		...
		public async Task<CommandResult<DesktopState>> ActivateAsync(DesktopSelector selector)
		{
			...
			return await this.InvokeOnOwnerAsync(() =>
			{
				...
				catch (Exception ex) { return this.Fail<DesktopState>("desktop_activate_failed", "The desktop could not be activated.", ex, desktop.Id); }
			});
		}
		...
		public async Task<CommandResult<DesktopState>> AddAsync(int position = 0)
		{
			...
			if (position < 0 || position > before.Desktops.Count + 1) return CommandResult<DesktopState>.Failed("invalid_position", $"Position must be 0 or between 1 and {before.Desktops.Count + 1}.");
			Guid createdId;
			try { createdId = await this.InvokeOnOwnerAsync(() => VirtualDesktop.Create().Id); }
			catch (Exception ex) { return this.Fail<DesktopState>("desktop_add_failed", "The desktop could not be created.", ex); }
			var created = await this.WaitForDesktopAsync(createdId, TimeSpan.FromSeconds(5));
			if (created == null) return CommandResult<DesktopState>.Failed("desktop_state_timeout", "The desktop was created, but it did not appear in the runtime state before the timeout.");
			...
			if (position != 0 && position != created.Position)
			{
				...
				catch (Exception ex) { return this.Fail<DesktopState>("desktop_insert_failed", "The desktop was created, but it could not be moved to the requested position.", ex, createdId); }
			...
			return created == null ? CommandResult<DesktopState>.Failed("desktop_state_unavailable", "The desktop was created, but its final logical state is unavailable.") : CommandResult<DesktopState>.Succeeded(created);
		}
		...
		public async Task<CommandResult<GlobalWallpaperState>> SetGlobalWallpaperModeAsync(ToggleAction action)
		{
			...
			return await this.InvokeOnOwnerAsync(() =>
			{
				...
				catch (Exception ex) { return this.Fail<GlobalWallpaperState>("global_wallpaper_failed", "Global individual wallpaper management could not be changed.", ex); }
			});
		}
		...
		public async Task<CommandResult<DesktopWallpaperState>> SetDesktopWallpaperModeAsync(DesktopSelector selector, ToggleAction action)
		{
			...
			return await this.InvokeOnOwnerAsync(() =>
			{
				...
				if (!Settings.General.ChangeBackgroundEachDesktop.Value) return CommandResult<DesktopWallpaperState>.Failed("global_wallpaper_disabled", "Global individual wallpaper management is disabled.", ToWallpaperState(this.CreateDesktopState(desktop)));
				try
				{
					...
					if (!target)
					{
						if (!desktop.HasWallpaper) return CommandResult<DesktopWallpaperState>.Succeeded(ToWallpaperState(this.CreateDesktopState(desktop)));
					...
					{
						if (!this._disabledWallpapers.TryGetValue(desktop.Id, out var remembered) || string.IsNullOrWhiteSpace(remembered.Path)) return CommandResult<DesktopWallpaperState>.Failed("wallpaper_unavailable", "No previously disabled individual wallpaper is available for this desktop.", ToWallpaperState(this.CreateDesktopState(desktop)));
				...
				catch (Exception ex) { return this.Fail<DesktopWallpaperState>("desktop_wallpaper_failed", "The desktop individual wallpaper mode could not be changed.", ex, desktop.Id); }
			});
		}

		private void OnRuntimeStateChanged(object sender, DesktopRuntimeStateChanged e) { this.RefreshViewModels(e.Change.Snapshot); this.RefreshAndPublish(); }
		...
		{
			if (this._dispatcher != null && !this._dispatcher.CheckAccess()) { this.PublishFromAnyThread(); return; }
			this.RefreshSnapshot();
			this.PublishSnapshot(this.GetState());
		}
		private void PublishFromAnyThread()
		{
			...
			if (dispatcher == null || dispatcher.HasShutdownStarted || dispatcher.HasShutdownFinished) { this.RefreshEnabledOnlySnapshot(); return; }
		...
		private void RefreshSnapshot()
		{
			...
			Volatile.Write(ref this._snapshot, new DesktopSystemState(this.Enabled, globalWallpaper, current?.Id, current?.CName, current?.Position ?? 0, desktops));
		}
		private DesktopState CreateDesktopState(VirtualDesktopViewModel desktop) => this.CreateDesktopState(desktop, this._runtime?.State?.CurrentDesktopId, Settings.General.ChangeBackgroundEachDesktop.Value);
		...
		private VirtualDesktopViewModel ResolveLiveDesktop(DesktopSelector selector, out string errorCode, out string errorMessage)
		{
			...
			else if (selector.Position.HasValue && selector.Position.Value >= 1 && selector.Position.Value <= this._desktops.Length) return this._desktops[selector.Position.Value - 1];
			errorCode = "desktop_not_found"; errorMessage = "The requested desktop was not found."; return null;
		}
		...
		{
			errorCode = null; errorMessage = null;
			if (selector == null) { errorCode = "invalid_selector"; errorMessage = "A desktop selector is required."; return false; }
			var count = (selector.Id.HasValue ? 1 : 0) + (!string.IsNullOrWhiteSpace(selector.CName) ? 1 : 0) + (selector.Position.HasValue ? 1 : 0);
			if (count != 1 || (selector.Position.HasValue && selector.Position.Value <= 0)) { errorCode = "invalid_selector"; errorMessage = "A selector must contain exactly one valid Id, CName, or one-based Position."; return false; }
			return true;
		}
		private static DesktopWallpaperState ToWallpaperState(DesktopState desktop) => new DesktopWallpaperState(desktop.Id, desktop.CName, desktop.Position, desktop.IndividualWallpaperEnabled, desktop.WallpaperPath, desktop.WallpaperPosition);
		private static bool ResolveToggle(ToggleAction action, bool current)
		{
			switch (action) { case ToggleAction.Off: return false; case ToggleAction.On: return true; case ToggleAction.Toggle: return !current; default: throw new ArgumentOutOfRangeException(nameof(action), action, null); }
		}
		...
		private async Task<DesktopState> WaitForDesktopAsync(Guid id, TimeSpan timeout)
		{
			...
			do { var found = this.GetState().Desktops.FirstOrDefault(item => item.Id == id); if (found != null) return found; await Task.Delay(25).ConfigureAwait(false); } while (DateTime.UtcNow < end);
			return null;
		}
		private async Task<DesktopState> WaitForDesktopAsync(string cName, int position, TimeSpan timeout)
		{
			var end = DateTime.UtcNow + timeout;
			do { var found = this.GetState().Desktops.FirstOrDefault(item => string.Equals(item.CName, cName, StringComparison.OrdinalIgnoreCase) && item.Position == position); if (found != null) return found; await Task.Delay(25).ConfigureAwait(false); } while (DateTime.UtcNow < end);
			return null;
		}
		...
		public void Dispose()
		{
			lock (this._gate) { if (this._disposed) return; this._disposed = true; }
